...
void *qemu_fdt_getprop_inherit(void *fdt, const char *node_path,
                       const char *property, int *lenp,
                       bool inherit, Error **errp)
{
    ...
    if (!r) {
        ...
        if (inherit && !qemu_devtree_getparent(fdt, parent, node_path)) {
            return qemu_fdt_getprop_inherit(fdt, parent, property, lenp, true,
                                            errp);
        }
	error_setg(errp, "%s: Couldn't get %s/%s: %s", __func__,
                  node_path, property, fdt_strerror(*lenp));
                              ...
                              bool inherit, Error **errp)
{
    ...
    prop = qemu_fdt_getprop_inherit(fdt, node_path, property, &len, inherit, errp);
    if (*errp) {
        return NULL;
    }
    while (cell) {
        ...
        if (!term) {
            error_setg(errp, "%s: %s/%s prop not null terminated",
    ...
    if (!*(char *)prop) {
        error_setg(errp, "%s: %s/%s out of bounds",
                   __func__, node_path, property);
        return NULL;
    }
...
uint32_t qemu_fdt_getprop_cell_inherit(void *fdt, const char *node_path,
                               const char *property, int offset,
                               bool inherit, Error **errp)
{
    int len;
    uint32_t ret;
    uint32_t *p = qemu_fdt_getprop_inherit(fdt, node_path, property, &len,
                                           inherit, errp);
    if (errp && *errp) {
        return 0;
    }
    if (len < (offset+1)*4) {
        error_setg(errp, "%s: %s/%s offset %d exceeds length %d",
                   __func__, node_path, property, offset, len);
        return 0;
     }
                                     ...
                                     int size, Error **errp)
{
    ...
    for (;size ;size--) {
        ret <<= 32;
        ret |= qemu_fdt_getprop_cell_inherit(fdt, node_path, property, offset++, false,
                                     errp);
        if (errp && *errp) {
            return 0;
        }
    ...
    for (;;) {
        ...
        if (info_dump) {
            ...
            if (fdt_get_path(fdt, offset, node_path, DT_PATH_LENGTH)) {
                ...
                all_compats = qemu_fdt_getprop_inherit(fdt, node_path, "compatible",
